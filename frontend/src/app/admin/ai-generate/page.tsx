  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-black">AI Quiz Generator</h1>
          <p className="text-black">Generate high-quality multiple choice questions instantly using Gemini AI.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-[var(--surface)] border border-[var(--surface-dark)] rounded-2xl p-6">
            <h2 className="text-lg font-semibold text-black mb-4">Generation Settings</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div>
                ...
                <select 
                  ...
                  className="w-full bg-[var(--background)] border border-[var(--surface-dark)] rounded-lg px-4 py-2.5 text-black focus:outline-none focus:border-primary-500"
                >
              ...
              <div>
                <label className="block text-sm font-medium text-black mb-1">Topic (Optional)</label>
                <select 
                  {...register('topic_id')}
                  className="w-full bg-[var(--background)] border border-[var(--surface-dark)] rounded-lg px-4 py-2.5 text-black focus:outline-none focus:border-primary-500"
                >
              ...
              <div>
                ...
                <textarea 
                  ...
                  rows={4}
                  className="w-full bg-[var(--background)] border border-[var(--surface-dark)] rounded-lg px-4 py-2.5 text-black focus:outline-none focus:border-primary-500 resize-none"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3 bg-primary hover:bg-[var(--primary-hover)] text-black rounded-lg font-medium shadow-md transition-all flex items-center justify-center gap-2 disabled:opacity-50"
              >
                {isGenerating ? (
          ...
          {generatedQuestions.length > 0 ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between bg-[var(--surface)] p-4 rounded-xl border border-[var(--surface-dark)]">
                <div>
                  <h3 className="text-lg font-bold text-black">Preview Generated Questions</h3>
                  <p className="text-sm text-black">{generatedQuestions.length} questions drafted. Review and edit before saving.</p>
                </div>
                <button
                  onClick={handleBulkSave}
                  disabled={isSaving}
                  className="px-6 py-2.5 bg-green-500 hover:bg-green-400 text-black rounded-lg font-medium transition-colors flex items-center gap-2"
                >
                    ...
                    {editingIndex === idx ? (
                      ...
                      <div className="space-y-4">
                        <textarea
                          ...
                          className="w-full bg-[var(--background)] border border-[var(--surface-dark)] rounded-lg p-3 text-black"
                          rows={3}
                        />
                        <div className="grid grid-cols-2 gap-3">
                          {editForm?.options.map((opt, oIdx) => (
                            <div key={oIdx} className="flex items-center gap-2">
                              ...
                              />
                              <span className="text-black font-bold">{opt.label}.</span>
                              <input
                                type="text"
                                value={opt.text}
                                onChange={(e) => {
                                ...
                                className="flex-1 bg-[var(--background)] border border-[var(--surface-dark)] rounded p-2 text-sm text-black"
                              />
                            </div>
                          ))}
                        </div>
                        <div>
                          <label className="text-xs text-black">Explanation</label>
                          <input
                            type="text"
                        ...
                        <div className="flex justify-end gap-2">
                          <button onClick={() => setEditingIndex(null)} className="px-4 py-1.5 text-sm text-black hover:text-black">Cancel</button>
                          <button onClick={saveEdit} className="px-4 py-1.5 text-sm bg-primary-500 text-black rounded hover:bg-primary-400">Done</button>
                        </div>
                      </div>
                    ) : (
                      // View Mode
                      <div>
                        <div className="flex justify-between items-start mb-3">
                          <h4 className="text-black font-medium text-lg leading-relaxed pr-8">
                            <span className="text-primary-500 mr-2">{idx + 1}.</span> 
                            {q.question_text}
                          </h4>
                          <div className="flex gap-2 flex-shrink-0">
                            <button onClick={() => startEdit(idx)} className="p-1.5 text-black hover:text-primary-400 transition-colors">
                              <Edit2 className="w-4 h-4" />
                            </button>
                            <button onClick={() => removeQuestion(idx)} className="p-1.5 text-black hover:text-red-400 transition-colors">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                          {q.options.map((opt, oIdx) => (
                            <div key={oIdx} className={`p-3 rounded-lg border ${opt.is_correct ? 'bg-green-500/10 border-green-500/30' : 'bg-[var(--background)] border-[var(--surface-dark)]'} flex items-center gap-3`}>
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${opt.is_correct ? 'bg-green-500 text-black' : 'bg-[var(--surface-dark)] text-black'}`}>
                                {opt.label}
                              </div>
